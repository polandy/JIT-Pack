...
/**
 * Ionic inputs hydrate late; filling before that goes nowhere. Private to
 * the container helpers below — a spec that fills its own fields elsewhere
 * keeps its own copy.
 */
async function fillContainerField(page: Page, testid: string, value: string) {
...
export async function createContainer(page: Page, name: string, limitKg?: string) {
  ...
  await fillContainerField(page, 'm11-name-input', name)
  ...
  if (limitKg) {
    await fillContainerField(page, 'm11-max-input', limitKg)
