...
### How many legs CI runs, and how to tell it has gone stale

`ci.yml` splits the suite over twelve legs with Playwright's `--shard=i/N`. The
count is a measurement (6249 test-seconds on 2026-09-20), and it goes stale
silently: nothing turns red, every run is just slower than it needs to be.

Two checks, cheapest first — the per-leg times of any recent run, and, when
those disagree wildly, a full report:

```bash
PLAYWRIGHT_JSON_OUTPUT_NAME=e2e-durations.json scripts/e2e.sh --reporter=json
```

A spread between legs is normal and bounded: `--shard` splits by test _count_,
so a leg that happens to hold the heavy end of `packing-list.spec.ts` runs
longer. Packing the legs by duration instead was built and rejected — it costs
a matrix edit per new spec file for about a minute (see the log entry "The CI
legs were split by counting, not by timing").
...
re-declaration of nine named helpers — **nine of the forty-two below**, so
...
| ----------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `helpers/page.ts`       | any screen                     | `visiblePage` (the one live `.ion-page`), `writesLanded` (the device has every write it made — the wait before any `page.goto` that follows one), `useReducedMotion`, `DESKTOP_BREAKPOINT`, `expectFiguresPaired` (two figures read as one: same ring, aligned side by side or stacked, no sentence cut short)                                                                                 |
| `helpers/ionic.ts`      | any form                       | `fillIonic`, `chooseInSelect`, `setDateField`                                                                                                                                                                                                                                                                                                                                                  |
| `helpers/trips.ts`      | M2 list, M3 wizard, composer   | `createTripViaWizard`, `createTripFollowingGroup`, `openTripFromList`, `expectTripOpen`, `openQuickAdd`, `addInComposer` (any typed add in M4/M6/M8 — through the create sheet when the name is new, FR-24.11), `confirmCreateSheet`, `createItemSheet`, `exactSuggestion`, `tripAction`/`tripActions`/`TRIP_ACTION`, `expectTripActionOffered`/`…Absent`, `openTripSwipe`, `tripSwipeActions` |
| `helpers/m4.ts`         | M4 packing list                | `tripWithRows`, `startTrip`, `packRow`, `row`, `openRowMenu`, `chooseInRowMenu`, `assignTraveler`, `openForWhom`, `lightTraveler`, `setMemberInM5`, `openTripTodos`, `addTripTodo`, `scrollPackList` (a wheel over the list, which since FR-21.17's gesture rule is the only scroll the head answers), `packListOffset`                                                                        |
