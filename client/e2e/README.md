...
| `helpers/trips.ts`      | M2 list, M3 wizard, composer   | `createTripViaWizard`, `createTripFollowingGroup`, `openTripFromList`, `expectTripOpen`, `openQuickAdd`, `addInComposer` (any typed add in M4/M6/M8 — through the create sheet when the name is new, FR-24.11), `confirmCreateSheet`, `createItemSheet`, `exactSuggestion`, `tripAction`/`tripActions`/`TRIP_ACTION`, `expectTripActionOffered`/`…Absent`, `openTripRowMenu`, `chooseTripRowAction`, `tripRowMenuActions`, `TRIP_ROW_ACTION` |
