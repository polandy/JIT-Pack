...
describe('master store — series tables from pull changes', () => {
  it('applies trip_series rows including parsed default attributes', () => {
    ...
    master.applyChange({
      seq: 1, table: 'trip_series', id: 'ser-1', deleted: false,
  ...
  it('applies destination profile and checklist items, keyed by series', () => {
    const master = useMasterStore()
    master.applyChange({
      seq: 1, table: 'destination_profiles', id: 'prof-1', deleted: false,
      row: { series_id: 'ser-1', notes: 'Waschmaschine vorhanden' },
    })
    master.applyChange({
      seq: 2, table: 'destination_checklist_items', id: 'chk-1', deleted: false,
    ...
    master.applyChange({ seq: 3, table: 'destination_checklist_items', id: 'chk-1', deleted: true, row: null })
    expect(master.getChecklistItems('prof-1')).toHaveLength(0)

    master.applyChange({ seq: 4, table: 'destination_profiles', id: 'prof-1', deleted: true, row: null })
