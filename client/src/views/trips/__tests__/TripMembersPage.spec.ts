...
/**
 * The FR-4.5 roster (`/trips/:tripId/members`) — ADR-033. Not M22, which is
...
describe('the FR-4.5 roster — an absence it has not read yet (ADR-033, G-7)', () => {
  it('says the roster is loading rather than saying it is not synced', async () => {
    ...
    expect(page.find('[data-testid="members-list-loading"]').exists()).toBe(true)
    expect(page.find('[data-testid="members-empty"]').exists()).toBe(false)
    expect(page.text()).toContain(t('members.listUnknown'))

    master.masterLoaded.value = true
    await flushPromises()

    expect(page.find('[data-testid="members-list-loading"]').exists()).toBe(false)
    expect(page.find('[data-testid="members-empty"]').exists()).toBe(true)
  })
