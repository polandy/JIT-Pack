...
describe('tripStore.getShoppingItems', () => {
  ...
  it('splits open items by procurement mode and drops packed ones', () => {
    ...
    tripStore.applyChange(itemChange('pack-item'))
    tripStore.applyChange(itemChange('before', { mode: 'buy_before' }))
    tripStore.applyChange(
      itemChange('before-packed', { mode: 'buy_before', state: 'packed', packed_count: 1 }),
    )
    tripStore.applyChange(itemChange('local', { mode: 'buy_local' }))

    const { buyBefore, buyLocal } = tripStore.getShoppingItems('t1')
  ...
  it('purchased BUY_BEFORE items leave the list once mode flips to pack (FR-3.3)', () => {
    ...
    tripStore.applyChange(itemChange('before', { mode: 'pack' }))

    const { buyBefore } = tripStore.getShoppingItems('t1')
...
describe('tripStore.getShoppingItems — what was bought (FR-25.11j)', () => {
  ...
  it('finds a purchased BUY_BEFORE row by the list it was bought from', () => {
    const tripStore = useTripStore()
    tripStore.applyChange(itemChange('before', { mode: 'pack', bought_from: 'buy_before' }))

    const { buyBefore, boughtBefore } = tripStore.getShoppingItems('t1')
  ...
  it('never counts a row as both open and bought', () => {
    ...
    const tripStore = useTripStore()
    tripStore.applyChange(itemChange('before', { mode: 'buy_before', bought_from: 'buy_before' }))
  ...
  it('nothing bought is an empty pair, not a missing one', () => {
    ...
    const { boughtBefore, boughtLocal } = tripStore.getShoppingItems('t1')
  ...
  it('carries bought_from onto the row it reads (FR-25.11j)', () => {
    const tripStore = useTripStore()
    tripStore.applyChange(itemChange('before', { mode: 'pack', bought_from: 'buy_before' }))
    tripStore.applyChange(itemChange('never', {}))
