...
/**
 * Create the item a search did not find (FR-24.11) — M9's inventory search,
 * and M10's companion picker (FR-20.1), which then declares the new item a
 * companion of the one being edited.
 *
...
const props = defineProps<{
  ...
  name: string
  /** Assigned from the start: on M9, the tags the list is filtered by. */
  tagIds: string[]
  /** Offered first: the tags the caller judges likeliest for this item. */
...
<template>
  <SheetModal :is-open="isOpen" testid="create-item-sheet" @dismiss="emit('dismiss')">
    <section class="sheet-body">
      <SheetHead
        :title="t('items.new')"
        title-testid="create-item-title"
        close-testid="create-item-close"
        @close="emit('dismiss')"
      />
      ...
      />
      <IonNote v-if="nameError" color="danger" class="field-error" data-testid="create-item-error">
      ...
      <div class="actions">
        <IonButton fill="clear" data-testid="create-item-open" @click="create(true)">
          {{ t('items.createAndOpen') }}
        </IonButton>
        <IonButton data-testid="create-item-confirm" @click="create(false)">
