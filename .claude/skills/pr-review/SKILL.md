...
- **Schema** (ADR-067, CLAUDE.md invariant 2): a schema change is **two edits** — `internal/store/schema.sql` *and* an additive `internal/store/migrations/NNN_*.sql` file. A schema-only or migration-only change is a finding, not a pass. Check the migration is genuinely additive (`CREATE TABLE`, `ALTER TABLE ADD COLUMN` appended, never a `DROP`/rewritten column in place) — SQLite has no real `DROP COLUMN`, so a destructive change costs a table rebuild written out in the migration file, never a silent one.
- **Breaking changes and the upgrade path — check every PR, not just schema ones.** Ask explicitly: does an existing on-disk database, a running older client mid-sync, or a documented wire/API contract break against this change? An additive migration file is evidence of safety but not proof of it by itself — a `CREATE TABLE` is safe by construction, but a column whose *meaning* changes under rows already on disk, a removed/renamed wire field, or a merge-rule change that reinterprets history is breaking even though the migration mechanics are fine. **From now on, the answer must be documented in the same PR**: if the change is breaking, or a clean upgrade needs a deliberate step (a data backfill, a one-time transform, a deprecation window), that step is written into `docs/upgrades.md` (or `dev-docs/Sync_API_Spec_v1.3.md` for a wire/API contract change) — never left to the generic upgrade mechanism to cover implicitly. If the change is purely additive and the existing generic mechanism (`docs/upgrades.md`'s "applies the schema changes it is missing and carries on") already covers it without a new sentence, say so explicitly in the §8 verdict rather than leaving the question unaddressed.
