...
| FR-5.10 finishing the packing · FR-30.8 the list that is now | E2E-M4-138 (the step asks, decides, says so, and one undo takes it all back including the stamp), E2E-M4-139 (variant P1: a half-packed row keeps what is in the bag — asserted on the trip's figure, which a skip would have left at 0/0), E2E-M4-140 (a finished list still takes an addition, and takes it as packed without reopening), E2E-M4-141 (*Wieder öffnen* lifts the stamp and leaves every decision standing — the half the undo does not cover), E2E-M4-142 (the last row packed raises the same question by itself, and *Later* changes nothing), E2E-M1-25 (the dashboard drops the packing figure and carries the finished line instead), E2E-M6-30 (M6 opens at the destination once the packing is closed, on a trip still *planning*) | `local` | [`close-packing.spec.ts`](../client/e2e/close-packing.spec.ts) |
