...
- [E2E-M9-19 — the case that found a read of state mid-change (2026-09-15)](#e2e-m9-19--the-case-that-found-a-read-of-state-mid-change-2026-09-15) — why a merge must not re-ask the guard it has just been writing past, and the alert input `fillIonic` cannot fill.
...
| M1 dashboard (populated) | E2E-M1-01 (card, open count, three previews, the remainder, and the card into M4), E2E-M1-02 (a row's preparation in the one *Aufgaben* card with its chip since FR-7.6 — resolved in M5, which clears it; listing only since 2026-09-18), E2E-M1-06/06b (the departure-day section), E2E-M1-07 (the chip on a task opens its row — the prep card's item name until FR-7.6; its head, count and card role since FR-21.28), E2E-M1-03b (no delegation section without accounts), E2E-M1-08 (the planned-trips section, and that starting a trip moves it; its head, count and card role since FR-21.28), E2E-M1-10 (FR-7.4 trip todos reported read-only, and the way into the trip), E2E-M1-11 (the task check and the packing share move independently) | `local` | [`dashboard.spec.ts`](../client/e2e/dashboard.spec.ts) |
...
| Global navigation & app bar | E2E-G9-09, E2E-G9-17, E2E-G1-06, E2E-G9-10, E2E-G9-11, E2E-G9-12, E2E-G9-13, E2E-G9-14, E2E-G9-15, E2E-G9-16 (UX-17 content column), E2E-G9-20 (FR-21.26 tablet-gap measure), E2E-G9-19 (ADR-050 a tab root's head, M1 included since FR-21.27), E2E-G9-21 (the build names itself once, header vs M17), E2E-G9-22 (M24 from M9's ⋮ and back, FR-24.12), E2E-G1-01 (partial), E2E-G1-02, E2E-G1-03, E2E-G1-04, E2E-G1-05, E2E-G1-07, E2E-G12-01 (partial), E2E-G12-02, E2E-G8-02, E2E-G2-02, E2E-G2-03, E2E-G2-08, E2E-G2-09, E2E-G7-02, E2E-G12-05, E2E-G12-06, E2E-G12-07 (since 2026-09-25 M6 and M25 carry no ⋮), **E2E-G12-08** (ADR-051 amendment 3: a held switcher glyph shows its name and goes nowhere — since 2026-09-25), E2E-G20-01 (G-20: a selection wears the app bar, the first row measured still), E2E-M3-15, E2E-M3-16, E2E-M4-32 | `local` | [`global-nav.spec.ts`](../client/e2e/global-nav.spec.ts) |
| M5 item detail | E2E-M5-09 … E2E-M5-14, E2E-M5-32 (the indicator's spoken half), E2E-M5-17, E2E-M5-05 (a note becomes a task), E2E-M5-23 (the companion offer), E2E-M5-31 (FR-7.3: the preparation is ticked at the end of its line, measured), E2E-G8-01 (no delegation picker), E2E-G4-01 (the notification's landing) | `local` | [`item-detail.spec.ts`](../client/e2e/item-detail.spec.ts) |
...
| M4 packing list | E2E-M12-06, E2E-M4-01, E2E-M4-04, E2E-M4-36, E2E-G6-02, E2E-M4-18 (both directions), E2E-M4-20, E2E-M4-21, E2E-M4-22, E2E-M4-23, E2E-M4-44, E2E-M4-45, E2E-M4-46, E2E-M4-47, E2E-M4-110/111/112 (FR-25.29: per-traveler rings add up to the trip line, taps pick several; none for one traveler; none during the closing pass), E2E-M4-107 (FR-24.11: a new name through the create sheet, offer above the partial hit), E2E-M4-108 (FR-24.11: the other umlaut spelling adds at once, then „schon drin"), E2E-M4-114 (FR-25.13j: the browse-sheet searches and creates a missing name in place), **E2E-M4-148** (G-14: the figures are a card lined up with the tasks card), **E2E-M4-147** (FR-25.13d/ADR-075: the browse sheet heads its groups with M9's own heading, count included — since 2026-09-24), E2E-M4-15 (partial), E2E-M4-02 (partial), E2E-M4-28 (partial), E2E-M4-56 (UX-9 name column, revised with the 2026-09-06 row), E2E-M4-68 (a done row sinks), E2E-M4-69 (FR-25.22: the reveal bar and the Erledigte switch carry one number), E2E-M4-57 (UX-13 bar overflow), E2E-M4-59 (FR-25.13e hide-carried), E2E-M4-60 … E2E-M4-63 (FR-25.13f: the browse-sheet's two verbs, on a free line and a carried one, and the line's own undo), E2E-M4-25 (+ E2E-M4-08, the prep lifecycle), E2E-M4-24 (the stamp's time, and that it clears), E2E-M4-11 (the shopping count), E2E-M4-19 (the shared bucket's word), E2E-G12-03, E2E-G12-04, E2E-G6-01 (the hold, and the row gesture that was swallowing it), E2E-G6-03 (a near miss beside the checkbox still packs the row), E2E-M4-66 (FR-20.4: the quick-add names the companions it pulled), E2E-M4-67 (FR-25.4a: only the unusual mode is drawn), E2E-M4-70 (FR-21.17: the page head yields with the header line, and holds at the bottom of the list), E2E-M4-135 (FR-21.17: a scroll nobody made moves neither the head nor the rows under it), E2E-M4-71 (FR-21.26: one content measure, kept by every screen the reader steps to), E2E-M4-72 (FR-21.19: a lone per-person row starts its name where every other row does), E2E-M4-73 (FR-21.20: a cluster head stands in the name column and its people step in), E2E-M4-74 (FR-21.22: the reveal bar wears a button's edge and says which way it goes), E2E-M4-75 (FR-21.23: the header line as a ring, a sentence and a track), E2E-M4-76 (FR-21.24: one door to the quick-add), E2E-M4-77 (FR-24.2: a generated row is filed under the item's tag), E2E-M4-83/84 (FR-25.13i: a settled line is reset after the sheet was reopened, and the decided-only filter that finds it), E2E-M4-85 (FR-25.11l: the Status facet overrides Erledigte for the picked bucket), E2E-M4-87 (FR-25.25: the late-packer flag set from the row's own menu, and the way back off it), E2E-M4-119 (FR-5.9: *Vor Ort kaufen* from the row's menu, read back on the row and in M6, and *Doch mitnehmen* back), E2E-M4-89 (FR-25.25/G-8: Local Mode offers no assignment seat — the negative half of E2E-M4-90), E2E-M4-93/94 (FR-25.27: a flagged row sinks to the end of its group, and the *Spätpacker* switch puts those rows away without letting the remainder read as done), ~~E2E-M4-96~~ (moved 2026-09-21 to E2E-M25-01, FR-7.7 — the trip's own tasks are written on M25 now), E2E-M4-97 (FR-7.4: the todos head the list, open while owed, with a figure in the header), ~~E2E-M4-105~~ (moved 2026-09-21 to E2E-M25-02, FR-7.7 — E2E-M4-106 still holds the rule for a preparation), E2E-M4-106 (the same for a preparation, ticked in the task section since FR-7.6), ~~E2E-M4-134~~ (moved 2026-09-21 to E2E-M25-03, FR-7.7 — which also asserts the *Meine* chip's absence; its positive half, ~~E2E-M4-133~~, moved to E2E-M25-05 in the server row), E2E-M4-136 (FR-7.6: a row's preparation stands among the trip's tasks under its own chip, counted in the one figure, and the chip leads to the row), E2E-M4-137 (FR-7.6/FR-5.8: removing the row takes its preparation with it, and the undo brings both back), E2E-M4-138 (FR-7.6/UI-Spec M4: both kinds of task are ticked at the row's own edge, measured against the packing row's control in the same frame), **E2E-M4-144** (FR-7.7: finishing the packing moves the open before-tasks to the trip itself, the question counts them off the same plan the write reads, and one undo takes rows and tasks back together — since 2026-09-21), **E2E-M25-07/08/09** (FR-7.8: a task carries one tag of its own, the groups it makes, the drag between them and the heading that refuses what it could not head — since 2026-09-21), **E2E-M25-01/02/03/04** (FR-7.7: M25, a trip's tasks in two phases — the first three moved here from M4, the fourth is the salve leaving the packing list and coming back — since 2026-09-21), ~~E2E-M25-10~~ (moved 2026-09-25 to E2E-M26-01, FR-7.13 — the notes have a view of their own), **E2E-M25-12** (FR-7.8/ADR-075: a hold selects instead of lifting, and the selection is tagged and moved to the other phase in one act each — since 2026-09-24), **E2E-M25-13** (FR-7.11: a due day set on the task's sheet wears its pill, leads its group and survives a reload, and Local Mode says it when the app opens — since 2026-09-25), **E2E-M4-149** (FR-7.12: finishing the packing moves both kinds of purchase to the destination under one undo, closes *before* on M6 and M25, and reopening lifts it — since 2026-09-25; M25's *before* read as the folded line at the end since FR-7.14), **E2E-M25-14/15/16/17** (FR-7.14: M25 reworked — a task filed with tag and day as it is typed and read in the *Fällig* block on top, the FAB to the field; words corrected on the sheet and *Erledigt* as its first act; the selection dated, ticked off and deleted in one act each; a trip under way files a new task for the road — since 2026-09-25), **E2E-M25-19** (FR-7.14/FR-30.8: a trip started ahead of its date takes nothing new *before* — M25's composer, sheet and selection, and M6's composer — since 2026-09-26) | `local` | **five files since 2026-09-20** — the unit was one file worth 15 % of the suite's test-seconds, which is what defeated every attempt to balance the CI legs: [`packing-list.spec.ts`](../client/e2e/packing-list.spec.ts) (the list and its rows), [`packing-list-sheet.spec.ts`](../client/e2e/packing-list-sheet.spec.ts) (M5 over the list, and what stays rendered), [`packing-list-shape.spec.ts`](../client/e2e/packing-list-shape.spec.ts) (head, line and measured layout), [`packing-list-adding.spec.ts`](../client/e2e/packing-list-adding.spec.ts) (how a row gets on), [`trip-tasks.spec.ts`](../client/e2e/trip-tasks.spec.ts) (FR-7.4/7.6, which is not a packing row at all) |
| FR-25.21 membership · FR-25.8 per-person quick-add · FR-25.28 the for-whom strip (every case in this row runs through the strip since 2026-09-18; the membership sheet is gone) | E2E-M5-18, E2E-M5-19, E2E-M5-20, E2E-M5-24 (FR-21.16: the head is set louder than its children, read off computed type), E2E-M5-21 (the state follows the numbers — implemented since 2026-08-30 and missing from this row until the M5 audit), E2E-M5-26 (FR-25.21c: *Alle* adds the missing travelers and keeps a chosen amount; its row-layout clause was retired with the sheet it measured), E2E-M4-12/E2E-M4-58 (one cluster, not N items), E2E-M4-14 (packing one instance does not flatten the other), E2E-M4-64 (G-8: the strip is absent), ~~E2E-M4-65~~ (retired 2026-09-18: no editor follows an add, so the sheet has nothing to make way for — E2E-M4-102 holds the opposite rule), E2E-M4-100 (FR-25.28: the seat unfolds a strip that follows its item from row to cluster; at most one), E2E-M4-101 (the last traveler leaves silently, progress kept), E2E-M4-102 (a browse-sheet add is deaf to the composer's strip), E2E-M5-29 (M5 closes with the row it stood on), E2E-M4-78/79 (FR-25.13g: „für alle" on a free line and on a carried one), E2E-M4-80/81 (FR-25.13h: assigning travelers, multi-select — the avatar buttons up to three, the long-press menu above), E2E-M4-82 (FR-25.23: the cluster folds, starts shut, and the shut head answers with faces and an open count), E2E-M4-113 (FR-25.30: filtered to one traveler the instance is a plain row, ticked without opening; clearing the filter restores the cluster), E2E-M4-88 (FR-25.26: the head sets the late-packer flag on every instance, asserted per instance in M5, and takes it back off), E2E-M4-117 (FR-25.26 widened: the head's *Menge* reaches each child, its skip takes the whole cluster and one undo returns it), E2E-M4-118 (the head removes every instance behind one undo), E2E-M4-132 (FR-5.9 from the head: every instance bought there, read per child in M5, *Doch mitnehmen* undone back per instance) | `local` | [`membership.spec.ts`](../client/e2e/membership.spec.ts) |
...
| FR-5.10 finishing the packing · FR-30.8 the list that is now | E2E-M4-139 (the step asks, decides, says so, and one undo takes it all back including the stamp), E2E-M4-140 (variant P1: a half-packed row keeps what is in the bag — asserted on the trip's figure, which a skip would have left at 0/0), E2E-M4-141 (a finished list still takes an addition, and takes it as packed without reopening), E2E-M4-142 (*Wieder öffnen* lifts the stamp and leaves every decision standing — the half the undo does not cover), E2E-M4-143 (the last row packed raises the same question by itself, and *Later* changes nothing), **E2E-M4-146** (FR-5.11: on a closed list the composer asks *packed* or *forgotten*; *forgotten* records a thing that stayed home — neither packed nor open, in its own words — since 2026-09-21), **E2E-M4-145** (FR-25.2/FR-25.11e: the reveal bar over what the close decided counts a skipped row under a word that is true of it — *„Show 2 done"*, the same in the other direction, and *„All done 🎉"* in the empty state above it — since 2026-09-21), E2E-M1-25 (the dashboard drops the packing figure and names the phase in the date line — no finished line since FR-7.10), E2E-M1-26 (the hero's task block adds, ticks, folds; adding to a folded block moves its count and not the fold), E2E-M1-27 (the shopping block adds and buys; no control inside a link), E2E-M6-30 (M6 opens at the destination once the packing is closed, on a trip still *planning*) | `local` | [`close-packing.spec.ts`](../client/e2e/close-packing.spec.ts) |
...
| Visual baselines | E2E-VIS-01 … E2E-VIS-14 (VIS-13 M25 since 2026-09-21, VIS-14 M26 since 2026-09-25 — the list and a thread's view) | `local` | [`visual.spec.ts`](../client/e2e/visual.spec.ts) |
...
| Every act on the list can be undone (FR-25.31) | E2E-M4-120 (un-pack), E2E-M4-121 (amount, ＋ step, confirmed removal), E2E-M4-122 (late packer from the menu), E2E-M4-123 (closing-pass mark), ~~E2E-M4-124~~ (moved 2026-09-21 to E2E-M25-06, FR-7.7 — a task is removed where it now lives), E2E-M4-125 (unskip, claim), E2E-M4-126 (cluster fan-out, in `membership.spec.ts`) | `local` | [`undo-every-act.spec.ts`](../client/e2e/undo-every-act.spec.ts) |
...
| Surfaces | E2E-G14-01, E2E-G14-02, E2E-G14-03, E2E-G14-04, E2E-G14-05, E2E-G14-06 | `local` | [`surfaces.spec.ts`](../client/e2e/surfaces.spec.ts) |
...
| M8 template editor | E2E-M8-01, E2E-M8-02, E2E-M8-03, E2E-M8-04, E2E-M8-05, E2E-M8-06 (its own test only since the 2026-08-30 audit), E2E-M8-07 (incl. E2E-M7-07's include half), E2E-M8-08, E2E-M8-10, E2E-M8-11 (editor half), E2E-M8-12, E2E-M8-13, E2E-M8-14, E2E-M8-15, E2E-M8-16, E2E-M8-17, E2E-M8-21, E2E-M8-22, E2E-M8-23 (two tests), E2E-M8-18, E2E-M8-24 (two tests), E2E-M8-26 (FR-7.4 trip tasks, one test per scope), E2E-M8-27 (FR-24.11: an unknown name becomes a position only through the create sheet), E2E-M8-28 (FR-28.8 on the fold row and the quick-add card) | `local` | [`template-editor.spec.ts`](../client/e2e/template-editor.spec.ts) |
| M6 shopping (own entries, packing rows by projection, FR-25.11j reveal, FR-25.6 aggregation, FR-30.4 purchase stamp) | E2E-M6-29 (`single`, `shopping/single/purchase-stamp.spec.ts`: the buyer named from the server), E2E-M6-26/27/28 (FR-30: an entry reaches no packing figure; bought, put back, removed across reloads; a packing row is listed exactly while its mode says so), E2E-M6-31 (FR-30.9: one tag per entry, the open list grouped by it, a flat reveal that names the tag, the check-off at the end of the row), E2E-M6-32 (FR-30.9: several entries, tagged or not, retagged in one act via an inline selection, entered by a long press or the header icon, with an undo), E2E-M6-33 (FR-25.11j: a bought row leaves smoothly with its own undo toast, ahead of the reveal), E2E-M6-34 (FR-30.9: a grip drags one entry into another tag via `useDragToGroup`, and refuses a packing-projected heading), E2E-M6-35 (FR-30.10: a due day set in the entry sheet — the pill, the entry in the *Fällig* block since FR-30.11, M1's card and Local Mode's opening hint), **E2E-M6-36** (FR-30.11: M6 reads like M25 — both lists on one screen, the *Fällig* block above them, the composer's list and day chips, an entry removed from its sheet — since 2026-09-26; the tab-, ✕- and reveal-reading cases revised the same day), E2E-M6-01/04, E2E-M6-17 (**with E2E-FLOW-03**, whose journey it already was — since 2026-08-31 it also reads the state the row arrives in), E2E-M6-22, E2E-M6-05, E2E-M6-06. ~~E2E-M6-21, E2E-M6-25~~ retired with M6's composer (FR-30.2) | `local` | [`shopping/shopping.spec.ts`](../client/e2e/shopping/shopping.spec.ts) |
| M9/M10 inventory & item editor | E2E-M9-01, E2E-M9-06, E2E-M9-05, ~~E2E-M9-08~~ (retired with the tag axis, FR-24.8), E2E-M9-10 (search filters), E2E-M9-14 (three chips + the filter sheet, FR-24.8), E2E-M9-15 (the jump, FR-24.8), E2E-M9-16 (the bulk refile, FR-24.9), **E2E-M9-31** (a hold selects, a tap picks, outside the mode a tap opens — FR-24.9/ADR-075, since 2026-09-24), **E2E-M9-26** (the bulk link, both directions and the undo — FR-24.9 widened/FR-20.1, since 2026-09-20), E2E-M9-17 / E2E-M9-18 / E2E-M9-19 (the tag manager: rename, the refused delete, the merge — FR-24.10), **E2E-M9-28** (several tags merged in one act — FR-24.14, since 2026-09-20), **E2E-M9-32** (the tag manager selects like the lists: a hold picks, the merge waits in the bulk bar — FR-24.14/ADR-075, since 2026-09-24), **E2E-M9-33** (a tag is dragged by its grip to another place on the axis — FR-24.10/ADR-075, since 2026-09-24), **E2E-M9-30** (two duplicate items merged into one, and the loser says where it went — FR-24.15/ADR-069, since 2026-09-20), E2E-M9-20 (the hidden items are named, FR-24.3 — lives in `restore-retired.spec.ts`), E2E-M9-21 / E2E-M9-22 (the search creates what it did not find, FR-24.11), E2E-M9-23 (a retired name is offered back, FR-24.11 — lives in `restore-retired.spec.ts`),  E2E-M10-21 (the chip's two targets, FR-24.9), E2E-M10-22 (the same while creating, FR-24.9), E2E-M9-11 (umlaut + tag search, FR-24.7), E2E-M9-12 (the filtered dead end, FR-24.7), E2E-M9-13 (the tool bar stays, FR-24.6), E2E-M9-04 (empty state → M15), E2E-M10-07, E2E-M10-08, E2E-M10-10 (renumbered 2026-08-30 — they ran as M10-01 … M10-05), E2E-M10-03, E2E-M10-04 (new 2026-08-30), E2E-M10-20 (new 2026-09-12), E2E-M10-23–28 (both dependency pickers create and restore, FR-24.11), E2E-M10-13 (German-seeded), E2E-M10-16 | `local` | [`inventory.spec.ts`](../client/e2e/inventory.spec.ts) |
...
| M2 row actions (the hold / right-click menu; a slide until 2026-09-24) | **E2E-M2-34** (the trip's properties and lifecycle steps are the row's alone; *Finish trip* opens M4's closing pass, 2026-09-25), **E2E-M2-19** (a right-click opens the menu, *Start* moves the trip off *Planned*, and a tap still opens a trip — since 2026-09-24), E2E-M2-06 (no Share without a session), E2E-M2-07 (export, both branches) | `local` | [`trip-list.spec.ts`](../client/e2e/trip-list.spec.ts) |
...
| Trip notes across identities (FR-7.9, since 2026-09-22; threads FR-7.13, since 2026-09-25) | ~~E2E-M25-11~~ (moved to E2E-M26-03), **E2E-M26-03** (a thread is new for a second member, counted on the notes pill, *Read* in the thread clears both, and the first note names who has read it — decision 3/4), **E2E-M26-04** (a reply re-opens a read thread for the other traveller with only the reply unseen and the divider above it; replying is not ticking; *Edit* in the menu for the author only), **E2E-M1-14** (M1's *Neue Notizen* card, amended: a thread by its newest unseen entry, ticked from the card, back with a reply, and its words open the thread's own view) — all three revised 2026-09-25 by the UX rework | `server` | [`server/trip-notes.spec.ts`](../client/e2e/server/trip-notes.spec.ts) |
| M26 trip notes (FR-7.13, since 2026-09-25) | **E2E-M26-01** (a view of its own, M25 one list again, a note written from the FAB's sheet, a card named by title or first line that shows its words, the code chip and the `tel:` link in the thread, the delete from the menu), **E2E-M26-02** (a thread read top to bottom with the field at the bottom, a reply lifts its thread and is quoted on its card, one level, the author's edit from the menu with *edited*, the delete naming its replies) — both revised 2026-09-25 by the UX rework | `local` | [`trip-notes.spec.ts`](../client/e2e/trip-notes.spec.ts) |
...
| the search reaches an umlaut name and a tag, and says which | E2E-M9-11 | **New 2026-09-13 (FR-24.7).** Both halves were *measured* against the family instance before the case existed: „gurtel" and „guertel" each returned 0 of 184 rows while „Gürtel" sat in the list, and a tag every row displays could not be typed. The heading assertion is what separates „it matched" from „it said why it matched". The fold's own arithmetic is `domain/__tests__/itemSearch.spec.ts`, whose three fold cases were run red against the old `name.toLowerCase().includes` rule before the new one existed. |
| a filtered dead end names the filter and offers the way out | E2E-M9-12 | **New 2026-09-13 (FR-24.7).** „socken" under the *Hygiene* chip — the audit's clearest screenshot, where the screen said only „Kein Artikel gefunden" while three socks were in the list. Pins the three facts the bare sentence withheld: which filter, how many hits lie outside it, and a control that drops it. The last assertion is the one worth having — the query **survives** the filter being dropped, because the user asked for socks and not for the inventory. |
| the tools stay put while the list scrolls | E2E-M9-13 | **New 2026-09-13 (FR-24.6).** Geometry on settled boxes, like M9-08: the bar's box before and after scrolling to the end, with the scroll offset read back as the positive signal that the list actually moved — without it the case passes on a page too short to scroll. The second half stacks the group heading *below* the bar rather than merely „visible", which is the difference between sticky and sliding underneath. **Proven red** against a build with `position: static` on the bar. |
| three chips, a sheet behind them, and two tags at once | E2E-M9-14 | **New 2026-09-13 (FR-24.8).** The axis is asserted **absent from the DOM**, not merely unused: a control that is hidden is a control that can come back. The chip's count and the *alle* mode are the two things the segment could not do at all. The footer's count is asserted against the list's, so the sheet and the screen cannot drift into two arithmetics. |
| the group heading jumps without filtering anything away | E2E-M9-15 | **New 2026-09-13 (FR-24.8).** Twelve rows on a 360 px viewport, because a list shorter than the screen makes the case vacuous. It asserts *where* the list landed (the group under the tool bar) and that all twelve rows are still there — a jump is not a filter. **What it cannot falsify is asserted as a unit**: the scroll must wait for the sheet's dismissal, and on a short list the clamp that rule exists for is invisible. |
| the inventory names the items it is not showing | E2E-M9-20 | **New 2026-09-15 (FR-24.3).** In the M23 unit, not M9's: retiring an item is the setup and that unit already owns the dance. A second item stays active throughout, or „the note appeared" would be satisfied by an inventory that had emptied itself. The tap is asserted through to M23's list, because the note's whole point is being the way there. |
| a missing name is offered beside partial hits, and the list survives creating it | E2E-M9-21 | **New 2026-09-18 (FR-24.11).** The query has a hit on purpose: an offer made only in the empty state would pass a case with none. The absence of the offer afterwards is read against the new row being rendered — the same write reaching both places. The toast geometry is on the settled box (`data-presented`), compared with the FAB's; the first render of the feature had it covering the button. |
| the filter's tag comes along, and „create and open" returns to the same search | E2E-M9-22 | **New 2026-09-18 (FR-24.11).** Asserts the chip pre-assigned *in the sheet* and the row answering the filter *after*, so neither half can be satisfied by the other. The way back is the header chevron, per the suite's rule for leaving a screen. |
| a retired name is offered back, not re-created | E2E-M9-23 | **New 2026-09-18 (FR-24.11).** In the M23 unit for E2E-M9-20's reason. No sheet opens — asserted as no presented modal, against the positive signal of the row returning — and M23's empty state closes the loop. |
| a tag is renamed, and a name another tag holds is refused | E2E-M9-17 | **New 2026-09-15 (FR-24.10).** Asserted on M9's **group heading**, not inside the sheet: the sheet renders a renamed row whether or not anything was written. The refusal clause reads the alert still being up and the heading behind it unchanged. Proven red against a build whose rename ignores the collision — „Kleidung" vanished and only „Technik" was left. |
...
| a hold selects a row, a tap picks, and outside the mode a tap opens | E2E-M9-31 | **New 2026-09-24 (FR-24.9, ADR-075 amended).** A real right-click rather than a dispatched `contextmenu`, because the defect worth guarding was the right button's own pointerdown arming the hold (fixed for M6/M25 in #583) — a dispatched event has no press. Every „nothing opened" is paired with a count that moved, so none is an absence. Written with the change, green first run in chromium; the row's navigation in code (instead of `routerLink`) is what every other bulk case in this file also crosses when it taps rows in the mode. |
| two duplicate items are merged into one | E2E-M9-30 | **New 2026-09-20 (FR-24.15, ADR-069).** The fixture is the assertion: the losing row carries a tag, a weight and a companion edge the survivor does not have, so a merge that only deleted a row would fail on three counts rather than none — and each is read where it is rendered, never off the list. The trip in the setup is not decoration: it is the reference that makes FR-24.3 *retire* the loser instead of removing it, which is the only state in which M23 can be asked whether it names the survivor. Written first without it, the case failed on an empty M23 — correctly, because nothing had referenced the row. The remark on the loser's row was added on review (2026-09-20): every other assertion here survives replacing M10's `mergedIdsOf(...)` with `[props.itemId]`, so the alias — the column ADR-069 paid a schema change for — had no case at the seam where a page uses it. Mutation-proved in both browsers before it was kept. |
| several tags are merged in one act | E2E-M9-28 | **New 2026-09-20 (FR-24.14).** The fixture is the reason the case exists: one item carries **two** of the three picked tags, which is exactly what a merge-per-pair gets wrong — both assignments re-pointed onto the survivor, refused by `UNIQUE (item_id, tag_id)` only once the push reaches the server. Green means one tag on that row afterwards, under the heading it already had. Three items and three tags rather than two and two, because with one source per item the whole plan collapses to the FR-24.10 case E2E-M9-19 already covers. The tag names are distinct words (*Sommerurlaub*, *Sommersachen*) and not case variants: the give idiom finds an existing tag under the uniqueness fold, so „sommer" typed beside „Sommer" assigns rather than creates, and the fixture would silently be two tags instead of three. |
| the tag manager selects like the lists | E2E-M9-32 | **New 2026-09-24 (FR-24.14, ADR-075 amended).** The right-click lands on the tag's **name**, the rename control outside the mode, so the case also says the row's hold takes it wherever it lands. No rename absence is asserted: a right-click sends no click, so it would hold whatever the ghost-click guard did — that half is `TagManagerSheet.spec.ts`'s, which drives the release click itself. The dimmed merge under one pick is paired with the lit one under two. |
| a tag is dragged to another place on the axis | E2E-M9-33 | **New 2026-09-24 (FR-24.10, ADR-075 amended).** Replaces the *hoch/runter* arrows no case had ever operated. Three tags, the last dragged before the first, so the move spans two places — one an arrow could not make. Waits on `data-drag` returning to `idle` rather than on the animation, and reads the order on M9's headings after closing the sheet. Mutation-proved: with the drop's `move` emit removed the headings stayed in their old order. |
| the inventory names who an item is usually for, and finds it by that name | E2E-M9-29 | **New 2026-09-20 (FR-1.9 over FR-24.4/24.7).** A `server` case because the property is not offered below two accounts (G-8) — the toggle's presence is part of the claim. The second item stays unassigned so „the row names Bob" cannot be satisfied by a list that prints a name on every row, and the search half is asserted through a **query that is the account's name**, which is what the FR offers instead of a filter axis. |
...
| a circular dependency is refused in words, and the reverse list shows the mode | E2E-M10-03 | **New 2026-08-30.** The *„Hängt ab von"* section has existed since §3.20 with no assertion on any of its rules: two other cases *drive* it as setup and M10-13 reads its heading. Pins the default mode, the read-only companion row, and the cycle named hop by hop — the refusal asserted against the companion row still being there, so „no dependency" cannot come from a page that rendered nothing. **Amended 2026-09-12:** the companion row stopped being read-only (FR-20.1), so the two assertions that it offered no select and no button were replaced by the mode it now shows. |
| a companion is declared, re-moded and removed from the main item | E2E-M10-20 | **New 2026-09-12.** The other end of FR-20.1's relation, which no case could have covered before because no screen wrote it. Every assertion crosses to the *other* item's editor: the edge is proved where it was not declared, which is what separates a stored relation from a drawn one. The cycle refusal is asserted from this direction too, against the dependent side still listing exactly one relation. |
| a companion the inventory lacks is created from the picker and declared at once | E2E-M10-23 | **New 2026-09-18 (FR-20.1/24.11).** Read back from the *new* item's editor, tag and dependency both — the picker's own row could be drawn without either write landing. The sheet is located as the **presented** one: M9 stays mounted underneath with a sheet of its own. |
| the companion picker offers a retired name back and declares it | E2E-M10-24 | **New 2026-09-18 (FR-20.1/24.11).** In the M23 unit, which owns retiring as setup. No presented modal, against the positive signal of the companion row; M23's empty state says the restore happened. |
| a retired companion that would close a circle stays retired | E2E-M10-25 | **New 2026-09-18 (FR-20.1/24.11).** The failure path of the restore offer. M23 still listing the item is the positive signal; mutation-proved by dropping the cycle check before the restore. |
| a main item the inventory lacks is created from the dependency picker and depended on | E2E-M10-26 | **New 2026-09-18 (FR-20.1/24.11).** The direction is the point: asserted on this item's dependency row and on the new item's companion list. |
| the dependency picker offers a retired name back and depends on it | E2E-M10-27 | **New 2026-09-18 (FR-20.1/24.11).** M23's empty state says the restore happened; no presented modal. |
| a retired main item that would close a circle stays retired | E2E-M10-28 | **New 2026-09-18 (FR-20.1/24.11).** Failure path; M23 still listing the item is the positive signal. Mutation-proved by dropping `mainRefused` before the restore. |
| a dependency's name leads to that item, from either list | E2E-M10-31 | **New 2026-09-25 (FR-20.1).** Owner request 2026-09-24: inspecting a dependent meant finding it again in the inventory. Only the name is a link — the row's mode select and remove button must not navigate — and both directions are walked, each landing asserted on the rendered editor (its head and its own list), not on the URL. |
...
| a picked bar becomes M4's facet, clearing the others | E2E-M12-04 + `usePackingFilter.spec.ts` (unit) | The e2e never has a second facet in force, so *„clearing the others"* could not fail there. |
| several picked bars become one facet, OR'd; a pick is taken back by a second tap and dropped by a dimension switch | E2E-M12-08 + `usePackingFilter.spec.ts` (unit) | Added 2026-09-19 when the tap stopped navigating. The third person's row is the positive signal that the list is narrowed. |
...
| **a traveller can be added *as* an account in one act** | **E2E-M22-14** (`server/multi-user.spec.ts`) | **New 2026-09-13.** A second case rather than a clause on M22-13: the traveller does not exist yet, so the account has to survive being created with them. The picker returning to „no account" is asserted too — a sticky value would make the next person the same account. |
| **an account added as a traveller in M3 is a member of the new trip** | **E2E-M3-24** (`server/multi-user.spec.ts`) | **New 2026-09-18.** Bob opening the trip is the assertion: it is readable to him only through a `trip_members` row, so the case fails against a wizard that linked the traveller and forgot the grant. The link itself is unit-asserted on the queued mutations (`tripCreation.seam.spec.ts`) — its order, after the generated rows, is invisible in a browser. |
| the link is written *after* the rows the add generates | `tripLifecycle.seam.spec.ts` | Not assertable in a browser: the symptom of getting it wrong is a notification count on a third device, so the order of the queued mutations is the assertion. |
...
**The related row was removed 2026-09-12** (owner: it read as noise rather
than a suggestion, most visibly under the *Diverses* catch-all tag).
`domain/quickAddChips.ts` now builds only the recents row; E2E-M8-21 keeps
its id and its wiring/positive-signal shape but drops point 1's tag claim
and point 3's tagged-inventory setup, since nothing left in the composer
keys on a primary tag. E2E-M4-46 is unaffected — it always pinned M4's
`excludeItemIds` wiring, not the row that read it.
...
## E2E-M9-19 — the case that found a read of state mid-change (2026-09-15)

FR-24.10's merge passed every unit test and failed this case on its first run, with the item still under the source's
heading and the source tag still there. The defect was in the action, not the case: `mergeTags` finished by calling
the *guarded* `deleteTag`, which re-counts the assignments — the ones the same loop had just been writing away. Where
those optimistic writes had not landed, the guard saw them, refused, and left behind a tag nothing carried.

**The rule it leaves:** a plan computed from the store is the authority for the rest of that act. Re-asking the store
halfway through is a read of state mid-change, and it answers about a world that no longer exists. `mergeTags` now
calls `mutations.deleteTag` directly — the plan already knows the tag is empty, and that is what makes it a merge
rather than a delete. The seam spec pins it with a merge whose every assignment is *dropped* rather than re-pointed,
which is the arrangement where the stale count is highest.

**Why no unit test saw it.** The seam context applies its optimistic writes synchronously, so the guard there always
read a store that already agreed with the plan. Only the built app has the gap. That is the second time on this
screen that a rule about *ordering* was invisible to everything except a rendered run — the FR-24.8 jump was the
first — and both times the fix was to stop asking twice rather than to wait longer.

**A mechanical note worth keeping.** A `promptText` alert's field is a plain `<input>` the overlay renders itself,
...
**E2E-M4-77** covers FR-24.2 on the trip side: a row generated from a group is filed under the master
item's primary tag. The path is the only one a user has — tag an item in M9, add it as a position,
follow the group into a trip — because nothing on M4 can set a category on a row.

**Membership is asserted through the heading's tally, not through containment.** A group heading is a
`<button>` and its rows are siblings of it, not children, so `toContainText('Badehose')` on the
heading can never pass; the first version of the case asserted exactly that and failed against a
*working* build. `Sommer 0/1` beside `none 0/1` says the same thing and is falsifiable: the unfixed
build has no `Sommer` heading at all and a bucket reading `0/2`.

**Mutation-proven** by making `withCategories` return `null` for every item, rebuilding, and watching
the case go red — then restoring and rebuilding. The rebuild is not optional: the suite drives the
built bundle, so a source-only edit proves nothing.
...
## M6 became a module, and its cases reach packing rows through M4 (2026-09-19)

FR-30 (ADR-066) split the shopping list off the packing list: entries typed on
M6 are rows of `shopping_entries`, and the packing list's buy-mode rows are
projected onto it. Three consequences for the suite:

**The file moved to `client/e2e/shopping/`**, the layout FR-29.9 names for the
planner. The `chromium`/`webkit` projects already take every `*.spec.ts` below
`e2e/` outside `single/` and `server/`, so no project config changed.

**Every buy row is now made on M4.** M6's composer was the fixture half the file
used to put a packing row on a shopping tab; it is gone, because M6 writes no
packing rows any more. `addBuyRowOnM4` (`helpers/m4.ts`) adds the row with the
composer and chooses its mode in M5 — the way a person does, and the path
E2E-M6-05/06 already took for the per-person item. E2E-M18-12 (the backup) uses it
too. Two ids went with the composer: E2E-M6-21 (its duplicate exclusion on M6)
and E2E-M6-25 (its create sheet on M6), struck through in the UI-Test-Spec; both
rules stay covered where the composer is.

**Three new cases, each with its positive signal.** E2E-M6-26's claim is an
absence — the entry is no packing row — so it reads M4's progress *before and
after* (0/1 both times) beside the missing `m4-row-Milch`. E2E-M6-27 reloads
...
## Two case ids kept their number and changed their promise (2026-09-20)

FR-7.6 joined a trip's two task lists into one, which deleted M4's *Vorbereitung*
section and M1's *Vorzubereiten* card. Four specs reached into those two surfaces,
and the four edits split into two kinds that are worth telling apart.

**Three were re-pointings.** `group-to-trip`, `trip-composition`, `item-detail` and
one case in `packing-list` opened the prep section to read a generated or promoted
preparation; they now open the trip's task section through the existing
`openTripTodos` helper and read the same task there. Two of them also asserted the
header's *„1 preparation open"*, which the header no longer says — the count moved
into the figure beside the share — so they assert the figure instead. Nothing they
promise changed.

**Two changed what they promise.** E2E-M1-02 said *the prep card lists open todos
grouped by item*, and E2E-M1-07 said *the prep card's item name opens the row*. The
card is gone; the promises are not, because both survive in the joined card — the
grouping as the chip, the jump as the chip's link. They keep their ids: a number
means what the suite implements, and a reader arriving from an old commit finds the
same id covering the same intent on the screen that replaced it. The spec entries say
what they used to assert and what replaced it, which is the whole reason not to
renumber.

**The trap this change had, and where it is asserted.** M4 hides a removed row before
its delete is written — FR-25.31's undo window — so the first build listed the tasks
of a row that was no longer on the screen, each carrying a chip into nothing. The
screen filters those out now, and E2E-M4-137 is what holds it: the task disappears
