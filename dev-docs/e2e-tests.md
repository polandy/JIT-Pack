...
|---|---|---|---|
| Harness smoke | E2E-M19-01 (partial), E2E-G7-01 | `local` | [`smoke.spec.ts`](../client/e2e/smoke.spec.ts) |
...
**Not yet covered:** everything else in spec §3 (global patterns G-1–G-11), §4 (M1–M20 beyond the above), §5 (cross-screen flows) and §6 (non-functional journeys). The `single` and `server` modes have no coverage at all — they need a real `jitpackd` harness and, for `server`, a mock IdP (spec §10 steps 3 and 5).
