...
| Trip notes across identities (FR-7.9, since 2026-09-22) | **E2E-M25-11** (a note is new for a second member, tickable, sinks and mutes once ticked, and the sheet names who — decision 3/4), **E2E-M1-14** (M1's *Neue Notizen* card, decision 1/2: lists the note, ticks it from the card, the card empties, and a second note's words lead into the trip) | `server` | [`server/trip-notes.spec.ts`](../client/e2e/server/trip-notes.spec.ts) |
