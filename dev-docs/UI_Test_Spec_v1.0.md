...
| E2E-G9-08 | G-9 List → detail → back | all | The everyday round trip entered through the trip list — the only path that reaches Ionic's cross-outlet transition. Uncaught runtime errors are asserted, minus a **known, pre-existing** Ionic transition error (see the implementation log), so a *new* one still fails the case. |
