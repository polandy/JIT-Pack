...
| E2E-G2-01 | G-2 Sync indicator | single/server | Glyph reflects synced → offline (queued count) as the network drops and returns to synced once the queue drains; tapping opens the detail, which states the queue and — inside a trip — leads to the conflict log. Outside a trip it says where the log lives instead of offering a dead button. *(Amended 2026-08-20 with the implementation: the transient `syncing` state is not raced — same reasoning as the M8-14 amendment — and the drain is asserted on the app's next own action (a trip open), because no reconnect loop exists today; the durable outbox / reconnect story is a separate work item, and its case extends this one.)* |
