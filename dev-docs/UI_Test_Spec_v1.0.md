...
* **E2E-M3-18** `all` (FR-2.6, *owed — not built, variant undecided*): step 4 lets a decision be made before the trip exists — at minimum dropping a row, which lands as FR-5.5 *skipped* (visible, struck through, reversible) rather than as a missing row; whatever else the chosen variant allows is asserted on the created trip, never only in the wizard.
