...
| FR-27.3 | E2E | M3-12 |
| FR-27.4 | E2E+UNIT | M8-05 (warning wording), M8-09 (offered → applied → M2 log), M8-19 (refused, and not asked again), M21-03, FLOW-09; `domain/refresh.ts` (`followsGroups` past/not-past, `declinePlan` per position, `proposedChangeCount` excludes bookkeeping), `composables/planningRefresh` (propose writes nothing, accept, decline), `views/trips/TripListPage.spec.ts` (both chips), `components/trips/GroupChangesProposal.spec.ts` (names every change, fold, decline note) |
