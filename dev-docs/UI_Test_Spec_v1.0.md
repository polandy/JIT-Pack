...
* **E2E-M18-10** `local` (FR-18.4, ADR-030) — **implemented** (`e2e/backup-restore.spec.ts`): the same backup, restored **twice** onto one device. The first run lands the trip and the restore list shows no *Schon vorhanden* mark; the second run marks the trip's row before the button is pressed, raises the toast that counts what it left alone, and leaves **exactly one** trip row and **exactly one** group. The count is the point: an assertion that no second row appeared would be just as green against a restore that deleted the first. *(Added 2026-08-24. Mutation-proved: with the identity check reading an empty trip list, the second restore produces two rows and the case fails on that count.)*
