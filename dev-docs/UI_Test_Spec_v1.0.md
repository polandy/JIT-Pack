...
* **E2E-M4-130** `server` (FR-7.5, added 2026-09-19) — **implemented** (`e2e/server/multi-user.spec.ts`): a trip
...
* **E2E-M4-131** `local` (FR-7.5, G-8, added 2026-09-19) — **implemented** (`packing-list.spec.ts`): Local Mode renders
...
| FR-7.5 | E2E+UNIT | M4-130 (the seat hands a todo over, the assignee is told and sees it on M4 and M1), M4-131 (no seat without a second account); `TripTodoList.spec.ts` (seat, read-only avatar, resolved), `comments.seam.spec.ts` (one field on the wire), `notificationrules_test.go` (who is told) |
