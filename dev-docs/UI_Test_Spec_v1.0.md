...
| E2E-G3-02 | G-3 Taking a row over | single/server | A row another device is packing offers exactly one action on press-and-hold — *Übernehmen* — and it confirms first, naming the holder and the row. Confirming leaves the row claimed by the *taker* (never free in between), the previous holder gets an FR-6.2 notification, and the trip records the takeover. Declining the confirmation leaves the claim with its holder. *(Rewritten 2026-08-24: this case used to advance the clock past `JITPACK_LOCK_TIMEOUT` and assert that the row stopped being locked. FR-5.7 removed the window, so the case now covers what replaced it — and the clock-advancing it needed is gone with it.)* |
