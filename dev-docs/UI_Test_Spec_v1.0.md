...
| FR-21.8 | E2E+UNIT+GATE | G14-01, G14-02 (the card is a plane and casts a flavour-correct shadow); surfaces.css (planes differ, `.jp-card` built from tokens, five radius steps, each cast written once); `scripts/design-tokens-gate.mjs` (no raw colour, radius or shadow anywhere in `client/src`) |
