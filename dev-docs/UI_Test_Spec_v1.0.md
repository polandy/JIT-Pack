...
* **E2E-M8-09** `all` (FR-27.4): a template used by a *planning* trip shows the blast-radius note naming that trip; adding/removing a position then puts the "⟳ N Änderungen aus Gruppen übernommen" chip **only** on that planning trip's M2 row (never on active/archived rows); the log lists each change with its source group. Up to ten changes it is written out under the row (the e2e case asserts that state); above ten it folds behind the chip, which the TripListPage component test pins from both sides of the threshold.
