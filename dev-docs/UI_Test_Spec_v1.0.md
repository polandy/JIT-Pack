...
| E2E-G2-08 | G-2 Sheet header alignment | local | The sheet's state glyph **starts on the same line as its title**, measured rather than eyeballed: the distance between the circle's centre and the title line's centre is at most 1 px. It was 14.5 px — `.head` aligned the circle to the top of the title *block* while the `h1` inside carried a 20 px top margin nothing had asked for, `.jp-sheet-title` naming a type role and no spacing at all. *(Added 2026-08-23. Geometry rather than the baseline beside it on purpose: a baseline reports that a pixel moved, this reports which rule broke. Variant A — resetting the margin alone — was measured too and rejected at +5.5 px: a 38 px circle and a 29 px line flush at the top cannot centre on each other, and the residual would move again with the title's size.)* **Revised 2026-09-07 (FR-21.12): variant A is what ships.** The rejection assumed this sheet owned its head; one head now serves nine sheets whose leads are 24, 38 and 44 px, so centring the lead on the title line is a per-sheet number again — the thing the shared head exists to remove. The case asserts the glyph and the title *start* on the same line, and still catches exactly the defect it was written for: a stray h1 margin pushes the line down and nothing else does (mutation-proved). |
...
| E2E-G2-15 | G-2 Why it says offline | single | The detail names **the last request that failed** — its method, its path and the status, or that nothing answered — so the state is readable off the screen instead of inferred (FR-19.6, added 2026-09-13). The boot pull is refused with a 503 *before the app starts*, which is the shape the family instance's iPad was in: glyph `offline`, nothing else to go on. The refusals are **counted**, so the case cannot pass in a world where no request was made at all, and the status and path are asserted untranslated — a diagnostic is copied, not read as screen copy. |
| E2E-G2-16 | G-2 Since when it says synced | single | The detail carries **the time of the last completed sync** (FR-19.6, added 2026-09-19): after a healthy boot the line is present and holds a time; in E2E-G2-15's refused-boot shape it is **absent**, because a sheet must not vouch for a connection the page never made. |
...
| E2E-G6-03 | G-6 Checkbox target | all | A tap beside a row's checkbox packs the row: once left of the glyph, in the control column that used to swallow it, and once just below the checkbox's own box, where it used to open M5. The click lands off the glyph on purpose — at its centre the case passes without the widened target too. Row-sized checkbox only; M5's large one is its own target. |
...
| E2E-G9-18 | G-9 Cold-start deep link, the data half | single | A trip **sub-screen** opened directly — a reload or a shared link onto `/trips/:id/shopping`, never having opened the trip — renders the trip's rows. The row is the whole assertion, and it is the positive signal for the absence beside it: M6's empty state is that list's `v-else`, so a rendered row is the one thing the failing build could not produce. New 2026-09-05 with U-10, and **red on the build before it**: only M4 pulled the trip partition, and every sibling relied on having been reached through M4. The mode is `single` on purpose — Local Mode hydrates the whole database at startup, so E2E-G9-06 beside it is green either way and cannot see this. ADR-033 is the doctrine: a partition that has not arrived is not an empty one. |
| E2E-G9-19 | G-9 A tab root's head comes from the frame | all | New 2026-09-06 with ADR-050. The three tab roots each wrote their own display-face `h1` into their content before it, so they are where "every head comes from the registry" could quietly stop being true — a title that simply vanished would redden nothing, none of them ever having carried a test id. Asserted on all three, with the **logo** present and no chevron as the positive signal that the head is the frame's rather than a drill-down's; and the import control, which moved from beside the name into the bar's cluster, is followed through to the screen it opens, because a move is only complete if it still works. **Extended 2026-09-09 (FR-21.27) to M1**, the fourth root, which had kept its greeting as an `h1` of its own — asserted as the head carrying the greeting and the subtitle as its meta line, with the screen itself visible, since the frame's head would stand there unchanged if the dashboard rendered nothing. |
| E2E-G9-21 | G-9 / M17 The build names itself once | all | New 2026-09-13. The bar's label was `v${__APP_VERSION__}` while the string already carried the tag's own `v` — `git describe --tags` and the release workflow's `APP_VERSION=${{ github.ref_name }}` both hand it over with one — so every build read `vv0.10.0-…`, the shipped image included. The unit that covered it asserted the component's template back to itself and would have passed against any prefix, which is why nobody caught it. The case asserts the **two surfaces agree**: the string in the app bar is contained in M17's About line. A prefix invented by one of them is then a mismatch rather than a screenshot nobody reads. |
...
| E2E-VIS-14 | Visual M26 — a trip's notes | all | **New 2026-09-25** (FR-7.13). The view that did not exist before: the notes pill current in the switcher's fourth place, a titled thread expanded — its first note, the reply field under it, *2 Antworten · neueste zuerst* and the two replies newest first, indented one step — and an untitled thread collapsed and named by its first line. Photographed after a reload, so the thread is read back from storage and no snackbar is in the baseline. Its decisions are pixels: the card per thread, the one-level indent, the field's place between the first note and the answers. |
...
| E2E-G13-05 | G-13 The count is beside the head, not inside it | all | On M11, the unassigned head's count is its own element: the UI face, smaller than the head, tabular figures. The promise is falsifiable only because the count was a separate element — joined into the label it would render identically to a reader and read as the head's own text to the assertion. |
...
| E2E-G14-05 | G-14 A segment with a side margin stays inside its column | all | ~~M25's Tasks/Notes segment and~~ M7's scope segment ends at or before its parent's right edge (M25's segment left with the notes, FR-7.13, 2026-09-25), at a 390 px phone and a 1180 px tablet viewport. Ionic sizes `ion-segment` `width: 100%`, so a screen's side margin pushed it past the column by the margin's width and the scroller cut its right end off straight — found on an iPad (owner, 2026-09-24), and 14 px past a phone screen too. The two screens are the two that give the segment a margin. |
...
| E2E-G9-11 | G-9/G-12 Reaching M11 and coming back | all | The luggage entry in M4's bar menu renders the container screen, M4's own actions do not survive the move, and back restores both the packing list and its cluster. **Extended 2026-08-19, rewritten 2026-09-06 (ADR-050):** the *head* switches with the screen as well — M11 names itself and puts the trip on its second line, M4 names the trip, and the registry is keyed per path, so a stale entry would leave M11's name standing on the packing list after back. The M11 unit exercises the screen; getting *to* and *from* it is a global pattern and lives here — the rule the working agreement added after four navigation defects that both green screen suites had missed. |
...
| E2E-G1-07 | G-1/ADR-012 The gear leaves one page in the outlet | all | Reaching a trip the way a user does — from M2's list, so the outlet holds two pages — and then tapping the gear. The outlet must show exactly one page afterwards, and M4's FAB must be hidden rather than merely absent, because Ionic keeps a stacked page mounted. E2E-G1-04 taps the same control and cannot see this: it arrives from the wizard, one page deep, which is too shallow for the leak. New 2026-09-13. |
...
| E2E-G12-08 | G-12 A glyph on the trip switcher names itself when held | all | **Implemented 2026-09-25 (ADR-051 amendment 3).** Holding the shopping glyph on M4 shows *Shopping* in a bubble; the release does **not** navigate — asserted after the bubble has gone on its own, so the packing list still being current is a settled answer rather than an early one — and a plain tap on the same glyph still does. The defect it guards is the click every hold's release still fires. |
| E2E-G12-07 | G-12 The trip's places are named in the page | all | **Implemented 2026-08-31; the clause has now been reversed twice and is met a third way.** As written: ~~Shopping (with open-item count), Luggage and Analytics sit on the trip title line and each lands in one tap~~ / ~~**No ⋯ exists**~~. The second fell on 2026-08-25 to UX-13 — M4 *does* carry a ⋮ for its once-per-trip actions (E2E-M4-57) — and the first on 2026-09-06 to ADR-050, which put the three destinations in that same menu. FR-21.21/ADR-051 paid that cost back on 2026-09-08 with **four** pills under the page's name — and ADR-051 amendment 1 split them on 2026-09-20. As pinned now (words narrowed to the current pill by ADR-051 amendment 3, 2026-09-25: every pill *named*, the current one in a word, the row measured at 360 px in its widest shape — five pills since FR-7.13): *Packliste*, *Einkaufen*, *Aufgaben* and, since 2026-09-25, *Notizen* are pills, *Gepäck* and *Auswertung* are words in the bar's ⋮ (asserted absent from the row and present in the sheet), the current view is marked with `aria-current` — including when it is one of the two, which join the row while you stand in them — and every view is reachable from every one — **since 2026-09-25 (ADR-051 amendment 2) the luggage and the analytics through the packing list**: M6, M25 and M26 have no ⋮ (asserted on each, on a rendered screen), so shopping → packing → luggage → analytics; M26's back is M4. The sideways step amendment 1 kept is gone by the owner's decision. |
| E2E-G12-06 | G-12 Icon-only is still nameable | all | **Implemented 2026-08-31, and it found two icons with no name.** Read against the screen the subject is smaller than it sounds: the four anchors carry visible labels in both presentations, so the unlabelled icons are the bar's own cluster — since ADR-050 M4's three destinations are words in its menu, and the case reads search, filter, fold-all and the ⋮ instead. `header-back` and `header-settings` carried `aria-label` and **no `title`**, so a pointer resting on the back arrow or the gear was told nothing; fixed with the case. The two names are asserted to agree, and the accessible one is read as a name rather than off the attribute — Ionic relays `aria-label` into its shadow button. **Since ADR-051 amendment 3 (2026-09-25) the trip switcher's glyphs are read too** (shopping, tasks and — since FR-7.13 — notes, off M4). A plain tap **navigates**. ~~and a long-press shows it as a bubble on touch~~ — **struck by the owner 2026-08-31**, not built and not to be: G-12's own ⋮ already answers it on touch, and in words. The one exception, the trip switcher, is E2E-G12-08's. **And the `title` half is narrower than the case makes it look**: measured over the source, only **9 of 62** icon-only buttons carry one, and the owner **narrowed the rule to the app bar** the same day, where the label was dropped to buy room. This case asserts the bar's names at runtime; the standing guarantee is `iconButtonLabels.spec.ts`, which reads the toolbar and whatever is slotted into it out of `AppHeader.vue` rather than from a list. |
...
| E2E-G12-05 | G-12 Literal icons | all | **Implemented 2026-08-31; read off the trip switcher since 2026-09-08 (FR-21.21) — since ADR-051 amendment 3 (2026-09-25) at any width, where a glyph is all a view you are not standing on shows, which makes this case load-bearing rather than decorative. Since ADR-051 amendment 1 (2026-09-20) two of the four are read inside the bar's ⋮ instead, which is the sharper version of the same rule: a reader who learned a glyph on a pill must not meet a different one in the menu.** Shopping, Luggage, Analytics and the Inventory anchor render four different glyphs — asserted as pairwise distinctness of the icon each button actually carries, including against the rail, because the trip's three neighbours are not the only glyphs the reader is holding in their head. Guards the regression where one generic glyph stood for several destinations, which defeats dropping the labels. |
...
* **E2E-M1-03b** `local` (FR-6.1, G-8) — **new 2026-08-31**: Local Mode carries no delegation section, and the
  ...
  rather than built.
* **E2E-M1-14** `server` (FR-7.9, added 2026-09-22; amended 2026-09-25 by FR-7.13) — **implemented**
  ...
  it. A reply brings the thread back, quoting the reply, and the words open **M26 on that thread, expanded**. *Revised:*
  the card listed a note by its words, and a second note's words led onto M4.
...
* **E2E-M5-25** `all` (FR-21.25, new 2026-09-08) — **implemented** (`e2e/item-detail.spec.ts`): the sheet of an item
  with no prep and no notes is shorter than 80 % of the viewport — it stood at a fixed 88 % — and unfolding *Details*
...
* **E2E-M17-15** `single` (FR-19.9) — **implemented 2026-09-13**, in `e2e/single/connection-repair.spec.ts`: the
...
* **E2E-M17-16** `server` (FR-19.9) — **implemented 2026-09-13**, in `e2e/server/logout.spec.ts`: an OIDC session is
...
* **E2E-M17-06** `local` (G-11/FR-21.3) — **implemented 2026-08-30**, in `e2e/settings.spec.ts`: the Appearance toggle
  switches the flavour, and Tag survives a reload. Written because **every other theme assertion in the suite seeds
...
* **E2E-M4-44** `all` (UI-Spec M4 / G-9, added 2026-08-19; **rewritten 2026-09-06, ADR-050**): the trip is named
  **exactly once**, in the **page head**, and the width decides nothing. It used to be named in two places by
  viewport — M4's header line below the breakpoint, the app bar above it, because beside six icons the bar rendered
  "S…" — and the bar names no page any more. The case asserts the head at 390 px and again at 1280 px, that M4's
  ...
  to a sub-screen and back, where the head states „Shopping" over the trip's name on its second line: the fact the
  composed „Shopping · Samedan" title used to carry in one string, at one size.
...
* **E2E-M26-01** `local` (FR-7.13, added 2026-09-25) — **implemented** (`trip-notes.spec.ts`): the notes are a view of
  their own. M25 is asserted one list with no segment, on a rendered screen; the notes pill leads to M26, marked
  current, with its empty state. A quick note is named by its first line and a titled one by its title, the newer first;
  the writer's own threads carry no tick (FR-7.9 decision 4, and G-8's Local Mode). Expanded, the quick note's sheet
  links its spaced phone number as `tel:` with the digits alone as the href; the delete, read back after a reload,
  leaves the other thread alone.
* **E2E-M26-02** `local` (FR-7.13, added 2026-09-25) — **implemented** (`trip-notes.spec.ts`): the thread's shape on one
  writer. Two replies sent from the field under the first note: the thread they answer moves above a newer one (question
  1) and says *2 replies*; inside it the first note stands first and the replies newest first; the thread offers exactly
  one reply field (one level). The author's ✎ edits title and words in place, and after a reload the thread is named by
  the new title and the entry says *edited*. Deleting the first note names its two replies on the button and, after a
  reload, takes them with it.
* **E2E-M26-03** `server` (FR-7.9/FR-7.13, added 2026-09-25, was E2E-M25-11) — **implemented**
  (`server/trip-notes.spec.ts`): a thread is new for a second member — marked *New* on the card and counted *1* on the
  notes pill's badge — and never new or tickable on its writer's screen (decision 4). The tick clears the mark and the
  badge; the first note's sheet then names the ticker, and only the ticker (decision 3). A third reader's "new"
  surviving a second reader's tick is `noteThreads`'s unit coverage and
  `TestStampActor_NoteAckUpsertCannotStealAnotherUsersRow`, both reading only the acting reader's own row.
* **E2E-M26-04** `server` (FR-7.13, added 2026-09-25) — **implemented** (`server/trip-notes.spec.ts`): Bob ticks Alice's
  thread, is offered no ✎ on her first note and one on his own reply (question 2). Alice sees his reply as new in her
  own thread, answers, and her thread is no longer new for her — replying is not ticking, but what she answered is
  behind her. Bob's pill then counts *1*, his thread is *New* again, and exactly one entry — Alice's newer reply, not
  his own nor the first note his tick reached — carries the unseen mark (question 3's rule, applied to a reply). Who a
  reply *notifies* is `TestPlanNotifications_NoteReply_ReachesTheParticipantsOnly_FR7_13`'s, a rule over participants a
  browser cannot see.
...
| FR-7.13 | E2E+UNIT+SERVER | M26-01 (a view of its own, M25 one list), M26-02 (a reply lifts its thread and lands on top, one level, the author's edit, the delete naming its replies), M26-03 (the pill's *neu* badge), M26-04 (a reply re-opens a ticked thread, ✎ for the author only), M1-14 (newest unseen entry, opens the thread), G12-06/07 (the fourth pill); `tripNotes.spec.ts` (threads, order, `seen_through`, own entries, edits), `TripNotesPage.spec.ts`, `notethreads_test.go` (one level, parent once, title dropped, author-only edit, cascade), `TestPlanNotifications_NoteReply_ReachesTheParticipantsOnly_FR7_13` |
...
| G-12 | E2E | G12-01…06 (app-bar placement, two clusters + no overflow, survives collapse, one line, literal icons, nameable glyphs); G12-08 (a held switcher glyph names itself and goes nowhere); G12-07 and M4-57 (since 2026-09-25: a ⋮ holds its own context — none on M6/M25, no trip-wide entries on M4) |
