...
| E2E-G14-05 | G-14 A segment clips its checked pill to the track's shape | all | Found on an iPad testing M25's Tasks/Notes segment (owner, 2026-09-23): Safari draws the checked button's own corner a hair off the track's radius exactly where the last button meets the track's rounded end, invisible in Chromium. The fix is `overflow: hidden` on `ion-segment`; this asserts the property computes on a rendered page, since the sub-pixel seam it closes cannot be proven from a browser that never drew it — the unit half (`theme/__tests__/surfaces.spec.ts`) only asserts the declaration is still there. |
