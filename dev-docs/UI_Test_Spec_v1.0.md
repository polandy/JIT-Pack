...
* **E2E-M22-04** `all` (FR-2.7): a trip that has started keeps its roster and refuses removal — the control is present and `aria-disabled`, with the reason rendered under the list rather than hidden, and adding still works. The absence of the ability needs the positive signal, which is the note.
