...
| [051](ADR-051_Trip_View_Switcher.md) | **The trip's views are a switcher in the page**: four pills inside the page head, rendered once by the frame from `meta.tripView`, the current one marked and inert — over promoting one view back onto the bar (answers driver 1 for one of three, and re-opens the argument the three-glyph budget settled) and over a trip-level tab bar (a second anchor model, 56 px that never collapse). Pays back the §3.25 cost ADR-050 wrote down, and adds the sideways step the ⋮ never had. Accepted cost: ~35 px of head, glyphs only from 480 px up, and `App.vue` importing one trips component (G-9, G-12, FR-21.21, FR-25.6) | Accepted |
