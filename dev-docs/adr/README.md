...
| [030](ADR-030_Trip_Identity_On_Import.md) | An imported trip is the same trip when its **year and name** match, decided in the client and reported per document, over a UNIQUE constraint (which parks the outbox), a `(import)` suffix (which labels duplication rather than preventing it) or a merge (FR-18.4) | Accepted |
