...
- **This digest is not Dependabot's.** Its docker ecosystem reads Dockerfiles and compose files, not shell scripts, so the pin is bumped by hand — the one pin in the repository that is. It lives in `scripts/playwright-image.sh`, sourced by this script and by `scripts/e2e.sh`, which moved into the same image on 2026-08-19 for an unrelated reason (its `--with-deps` install was 63 % of the e2e job); a bump therefore moves the behaviour suite and the baselines together, which is correct — both have to match `@playwright/test` in the lockfile. That cuts both ways and is worth stating rather than glossing: the image can fall behind silently, and equally, a bump cannot arrive unannounced on a Tuesday and rewrite every baseline. Invariant 8 in `CLAUDE.md` names the exception so the general claim there stays true.
...
- **The image is pinned by digest *and* by platform** (`--platform linux/amd64`, added 2026-08-16). The digest alone fixes what is in the image, not what it is compiled for: on an Apple-Silicon machine docker resolves the same digest to its arm64 variant, and a baseline recorded there would be judged in CI against a rendering it never saw. Naming the platform is free on the runner, which is already amd64, and it is what makes the images *generatable* on a development machine instead of only on the runner. Verified when M11 joined the set: all 16 existing baselines reproduced byte-identically under emulation, so the pin is a real guarantee rather than a hope. The pin alone was not enough to make a development machine work, though — mounting the worktree also hands the container the host's `node_modules`, whose native binaries are the host's platform, so `scripts/visual.sh` gives the container its own dependency tree, out of the user's cache directory rather than the worktree, whenever the host is not Linux. On the runner nothing changes.
