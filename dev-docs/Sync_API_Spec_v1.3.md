...
| `GET /templates/{id}/export` · `POST /templates/import` | Addendum FR-18.2/18.4: portable YAML template export/import. A Ferien-Vorlage whose name the owner already uses is refused with `409` `name_taken` — `templates` is UNIQUE on (owner_id, name), and the alternative was an opaque `500`. A **group** document of a taken name is linked rather than refused (FR-27.1/ADR-017). The `(import)` suffix that keeps both is a client behaviour on the restore path, never this endpoint's |
