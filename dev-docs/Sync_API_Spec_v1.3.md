...
| `GET /templates/{id}/export` | Addendum FR-18.2: portable YAML template export. **There is no import endpoint** — `POST /templates/import` was removed 2026-08-23 (ADR-025). It was a second implementation of the import rules, and it wrote rows without a change-log entry, so what it imported reached no client. Importing writes ordinary mutations through `POST /sync/master` and `POST /sync/trips/{id}` like everything else |
| `GET /trips/{id}/export.yaml` | Addendum FR-18.3: portable YAML trip export (`POST /trips/import` removed with its template twin, ADR-025) — distinct from `export.csv` below (NFR-4.5), which is a flat data dump, not round-trippable. Export requires trip membership (403 `forbidden` otherwise), same as `export.csv` and the conflicts endpoint |
