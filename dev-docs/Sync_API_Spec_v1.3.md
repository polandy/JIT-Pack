...
* Locks (`packing_now`) are **also** persisted via normal mutations; the ephemeral event only lowers latency. Offline devices converge via pull — a stale lock older than the window is ignored by clients (timeout rule). **Decided: 15 minutes is the shipped default, configurable via an environment variable** per the declarative-config principle (Section 2), not adjustable from within the UI. **The client enforces it and the server does not** (owner decision 2026-08-23): the instance publishes its window on `GET /config`, and every device applies it — no push is refused for a foreign claim, and no server-side timer clears one. Local Mode has no server at all, so a rule the server kept would hold in two modes out of three. A claim therefore ends by being packed, by being **released** by the device holding it, or by ageing out of the window; the row keeps `packing_now` in either of the last two cases until something writes over it, which is why the client says an aged claim is aged rather than letting the row go quiet.
