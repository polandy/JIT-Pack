...
- **The e2e shard count is a measurement, not a constant.** It is 10 (`ci.yml`), sized 2026-09-18 against ~2940 test-seconds; a growing suite makes it stale silently, so when e2e feels slow read the per-leg times of a recent run first. **A single spec file that is a sixth of the suite defeats the count-split** — that is what M4's unit was until it was split into five files on 2026-09-20 (log: 'One spec file was 15 % of the suite').
