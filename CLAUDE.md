...
- **Verify before finishing any change: `make ci`** — mirrors the CI jobs 1:1 (gofmt, build, vet, race tests, coverage gates, golangci-lint, client lint/build/vitest). Budget ~3 min on an idle machine; **read the load average before trusting a timing** — a parallel session has turned a 100 s vitest run into 370 s.
...
- `internal/store` — the only package importing `database/sql`. SQLite repositories, change/conflict logs, the two sync partitions (master; trip), schema applied from `schema.sql` and fingerprinted in `PRAGMA user_version` (ADR-018).
...
2. **The development phase has no DDL migrations** (ADR-018). `internal/store/schema.sql` is the whole schema, always current: a schema change **edits that file**, with no upgrade path. `Open` applies it to an empty database and stamps a fingerprint into `PRAGMA user_version`; any other value is refused with `ErrSchemaStale` naming the database path. **Nothing is recreated or deleted on start-up.** Consequences: a schema change means deleting every development database (the `:3000` instance too — reseed with the M2 dev button), and a data transformation has nowhere to live, so a change needing a backfill is a reseed. **This reverts at the first release meant for anyone but the maintainer** (`schema.sql` → `migrations/001_schema.sql`, numbering resumes at `002`); the trigger is in ADR-018.
3. **The client's identity claims are never trusted.** The server stamps actor columns itself (`stampActor` in `internal/api/server.go`: comment `author_id`, `packing_now_by`/`packing_now_at`, `packed_by_user_id` — also stripped from incoming `trip_items` mutations). `packer_user_id` is deliberately *not* stamped: since FR-25.19 it is the assignment. A client placeholder like `'current-user'` must never reach a foreign key. Clients can never grant `owner`, and the trip creator's membership row is immutable.
...
- Don't add a migration (invariant 2) — `.claude/settings.json` denies creating `internal/store/migrations/**` and `.github/hooks/jitpack.json` guards Copilot (`dev-docs/agent-tooling.md`).
