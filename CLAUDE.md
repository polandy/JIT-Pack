...
6. ~~**Vue 3 + Capacitor client**~~ — **IN PROGRESS.** 153 client tests passing. Built so far:
   ...
   - **Not yet built:** M11–M18 (P2 screens). OIDC login flow UI. M8's editor mutations are still store-local placeholders — wire them through the orchestrator to the master partition (backend endpoints exist now). M3 deferred bits: series picker (FR-13.1), sharing/role step (FR-4.5), history suggestions (FR-14.2).
