...
- **The e2e legs are two measurements, not constants.** Ten legs (`ci.yml`, sized 2026-09-18), and since 2026-09-20 each leg *names its spec files*, packed by measured duration (`scripts/e2e-shard-plan.mjs`; `e2e-shard-plan-gate.mjs` in `make ci` refuses a file no leg runs). Both go stale silently — when e2e feels slow, read the per-leg times of a recent run first, then re-pack (`client/e2e/README.md`).
