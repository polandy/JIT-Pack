...
6. ~~**Vue 3 + Capacitor client**~~ — **IN PROGRESS.** 143 client tests passing. Built so far:
   ...
   - **Not yet built:** M3 Trip Creation Wizard UI (domain layer for it is done, see above), M6 Shopping Views, M11–M18 (P2 screens). OIDC login flow UI. M8's editor mutations are still store-local placeholders — wire them through the orchestrator to the master partition (backend endpoints exist now).
