...
- **The e2e leg count is a measurement, not a constant.** Twelve (`ci.yml`), re-sized 2026-09-20 against 6249 test-seconds; twelve is the last leg that still starts at once under the 20-job ceiling. A growing suite makes it stale silently, so when e2e feels slow read the per-leg times of a recent run first (`client/e2e/README.md`).
