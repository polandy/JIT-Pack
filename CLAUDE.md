...
5. **Three modes, one artifact** (ADR-009). Behaviour is selected at runtime by `jitpack_mode`, never by a separate build. Every feature must answer: what happens in Single-User Mode (auth and membership are bypassed — anything gated on `authed` is inert) and in Local Mode (no network)? Server-only surfaces are hidden per G-8, not left broken.
