...
* **Default ordering (Addendum §3.25, proposed 2026-07-17):** the list is **sorted by date, newest trip first** — *not* grouped by Trip Series by default (concept-review decision). Series grouping (FR-13.1) becomes an **optional view** the user can switch to; the tappable series header → M16 lives there.
...
* **⚠ Redesign pending (Addendum §3.25, proposed 2026-07-17 — to be re-mocked from scratch):** concept testing found the screen must give far more room to the actual packing. Binding changes for the redesign: **(a)** *decided (2026-07-17):* the header is a **single compact status line** (trip · packed/total · weight · prep) with tools (Shopping/Containers/Analytics) behind an **⋯ overflow** and the grouping switcher below; on scroll-**down** the sub-header hides and the **trip name + presence facepile migrate up into the top app bar** (collapsing large-title pattern), returning on any upward scroll; the global top app bar is a short single-line title (no subtitle, compact height, lightweight line-mark logo); **(b)** keep the inline quick-add but **collapse it on blur** and make the **bottom-right ＋ FAB expand+focus** it (FR-25.5-directive); **(c)** **hide completed *and* consciously-skipped items by default**, with an unobtrusive "N gepackte anzeigen" reveal (a dashed toggle at the list foot) that shows the done rows **dimmed (~45 % opacity)** and still interactive; a row is *done* only when fully packed **with no open prep**, group headers keep `done/total` over the full set, and a fully-packed group collapses header-and-all (FR-25.2 — realised in the concept 2026-07-17); **(d)** *decided (2026-07-17, supersedes the earlier flat-rows choice):* for per-person items, render a **named cluster** — the item name once as a sub-header with a `done/total` count, one **indented child row per traveler** (avatar + name + own check/stepper), each independently checkable; a per-person item contributing only a single instance to the current group (esp. when **grouped by traveler**) falls back to a flat "Item · Person" row; the cluster obeys the FR-25.2 hide-done rule (done instances drop out, cluster vanishes when all done) (FR-25.1); **(e)** *decided (2026-07-17):* show the **packer's avatar at the right edge** of any packed row — set apart from the traveler avatar (left, "for whom") by a **green ring + check badge**, appearing as soon as anything is packed (incl. a partially-packed stepper row) and, on a per-person cluster, on each packed child row; it replaces the former "gepackt von …" subtitle text (FR-25.3 — realised in the concept 2026-07-17); **(f)** *decided (2026-07-18):* mark procurement mode with a **row glyph on the two buy modes only** (🛒 Vorher kaufen · 📍 Vor Ort kaufen; 🧳 Packen stays icon-free so the must-buy exceptions stand out — on a per-person cluster the glyph sits once on the cluster header), and offer a **multi-select mode filter** as its own pill row below the grouping switcher (Alle · 🧳 · 🛒 · 📍, each with a live count, the mode pills combinable e.g. Vorher + Vor Ort, Alle clears it); **Late Packer** is kept as a *separate* ⏰ flag, orthogonal to the mode (FR-25.4 — realised in the concept 2026-07-18); **(g)** container assignment defaults to none and is de-emphasized so it never blocks packing (FR-25.5); **(h)** *decided (2026-07-17):* M4 runs **full-screen** — the **bottom tab bar is hidden** on this screen (packing is a drilled-into trip-detail; the ‹ back chevron returns to Reisen), the **＋ FAB drops to the screen foot** and the list padding tightens (chosen over auto-hide-on-scroll and an explicit focus toggle; desktop nav rail unaffected). The bullets below describe the *current* prototype and will be superseded by the redesign.
* **Elements:**
...
* **Concept-review refinements (Addendum §3.25, proposed 2026-07-17):** (1) **Delegation must be reversible** — clearing *Packed by* / undoing a delegation needs an obvious affordance (currently not discoverable, FR-25.5-directive). (2) **Container picker de-emphasized, default none** — it must not read as a required step (FR-25.5). (3) **Visible comment composer** — the FR-7.1 thread needs an explicit "add comment" input on this screen (missing today). (4) **Mode labels clarified** to 🧳 Packen · 🛒 Vorher kaufen · 📍 Vor Ort kaufen, with **Late Packer** presented as a *separate ⏰ flag*, not a fourth mode (FR-25.4). (5) **Preparation lifecycle made explicit** — add / resolve / reopen, and the "packed with open prep" (amber) state, so the section is self-explanatory.
* **Elements:** Header: name, quantity stepper, state; assignment section: *Used by* (traveler picker) vs. *Packed by* (user picker, **with a clear un-delegate/clear action** per §3.25) — visually distinct to reinforce FR-4.2; mode selector (🧳 Packen / 🛒 Vorher kaufen / 📍 Vor Ort kaufen, FR-3.1); **optional** container picker, default none (FR-10.2, de-emphasized per §3.25); Late Packer flag (⏰, distinct from the modes); flags *Unused/Missing* (FR-9.1, visible only on active trips); history sparkline: quantities in previous series trips (FR-14.1); comment/task thread (FR-7.1/7.2) **with a visible comment composer** and "flag as task" toggle per comment; **Preparation Todos section (FR-7.3):** dedicated section listing the item's prep todos with checkbox per todo, inline "Add prep todo..." input at the bottom; all trip members see todos, only assignee/owner can resolve. Packed items with open todos show an amber state indicator and "open prep" label.
* **Actions:** Delegate → triggers notification (FR-6.2); **un-delegate / clear *Packed by*** (§3.25); add a comment; resolve/reopen tasks; add/resolve/reopen preparation todos (FR-7.3); "Buy now" on *Vorher kaufen* items → mode flips to *Packen* with undo snackbar (FR-3.3). In Single-User Mode (Addendum FR-17.3), *Delegate* is hidden — the sole user is already every item's *Packed by*.
