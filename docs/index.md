...
The [Getting Started walkthrough](getting-started.md) takes it from nothing to a running instance; [Installation](installation.md) covers the reverse proxy that has to sit in front of it.

## How it fits together

JIT-Pack is two pieces and one file:

- **`jitpackd`** — the Go server. It owns the SQLite database, the sync protocol, and the OIDC login broker. It serves **only** the API; it does not serve the client.
...
| [Family Setup](family-setup.md) | From a running multi-user instance to a household actually using it. |
...
| [Upgrades](upgrades.md) | Moving between pre-1.0 versions without losing the family's data. |
