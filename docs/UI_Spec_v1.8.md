...
* **Elements:** Greeting header with sync glyph (G-2); grouped card list per active trip: my open packing items (count + next 3), my open tasks (FR-7.2), items delegated to me since last visit (highlighted); global "Late Packer" section that appears only on a trip's departure day.
...
* **Step 2 — Travelers:** Add travelers (name + Adult/Child, FR-2.5), optionally link to a registered user account; share the trip with user accounts and assign roles (FR-4.1/4.5). In Single-User Mode (Addendum FR-17.3), the sharing and role-assignment part of this step is hidden entirely — only traveler add/edit remains, and the sole user is silently the trip's Owner.
...
* **Elements:**
  * Sticky header: trip name, KPI strip — packed/total items, weight packed/planned (kg), value (CHF/EUR) (FR-8.1); tap KPI → M12; trip presence facepile and group-sync badge per G-10.
  ...
  * Collapsed sections: "Consciously skipped" (quantity 0 items, FR-5.5) and "Late Packers" (pinned to bottom until departure day, then pinned to top).
  * Filter bar: my items only, open only, per traveler.
* **Actions:** Swipe right → *Packing Now* (FR-5.2); swipe left → assign-to-me (FR-4.3); tap row → M5; long-press checkbox → complete item; toolbar: add ad-hoc item (auto-flagged *Missing* if trip is active, FR-9.1), open shopping views (M6), start Repack (M13, only when trip is active), archive trip (→ triggers M14).
