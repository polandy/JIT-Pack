...
#
# Arguments are passed to `playwright test`. CI passes `--project=P --shard=N/M` (M from
# its job's matrix length), one per leg; `make e2e` passes nothing and runs the whole suite.
set -euo pipefail
